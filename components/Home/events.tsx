...
export default function Events(){
    ...
    return(
        <div className="bg-black p-10 pb-20  " >
            <div className="flex flex-col gap-4 text-white items-center justify-center text-center mb-10 " >
                <h1 className="font-heading " >Upcoming Events</h1>
                <p className="font-secondary " >Join us for gatherings that bring our church family together.</p>
            </div>
            <div className=" grid grid-cols-3  mx-auto items-center gap-5  w-[95%]  " >
                {events.map((event)=>(
                <div key={event.id} className=" rounded-xl bg-[#1E1E1E] " >
                  ...
                  <div className="p-5 flex flex-col gap-2 ">
                      <h3 className='font-heading-secondary text-white' >{event.program} </h3>
                    <p className="font-family-figtree font-normal text-[14px] text-[#FFDD55] " >{event.date} </p>
                    <p className="font-family-figtree font-normal text-[14px] text-white " >{event.desc} </p>
                  </div>
                </div>
                ))}
            </div>
           ...
           <div className="flex items-center justify-center mt-8 " >
             <button className="text-white py-2.5 px-[6]  border-white border-[0.5px] rounded-lg heading-primary " >Learn More</button>
           </div>
        </div>
    )
}